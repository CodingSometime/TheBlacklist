...
class GroupCompanyModel extends BaseModel
{
  ...
  protected $primaryKey = "ID";
	...
	public function selectBox($controlName=null, $selected = null, $hasAll = false, $isReadOnly = false)
	{
		...
		$this->db->select("ID AS OPTION_VALUE, CONCAT(GROUP_NAME, ' (', GROUP_CODE, ')') AS OPTION_NAME", false);
	...
	public function datalistBox($controlName = null, $selected = null, $hasAll = false, $isReadOnly = false)
	{
		...
		$this->db->select("GROUP_CODE AS OPTION_VALUE, CONCAT(GROUP_NAME, ' (', GROUP_CODE, ')') AS OPTION_NAME", false);
		$this->db->where("STATUS_ID", 1);
		$this->db->order_by(2);
		$query = $this->db->get($this->tableName);
		$results = ($query->result_array());
