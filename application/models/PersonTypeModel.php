...
{
	protected $tableName = "FND_PERSON_TYPE";
	protected $viewName = "FND_PERSON_TYPE";
	...
	public function datalistBox($controlName = null, $selected = null, $hasAll = false, $isReadOnly = false)
	{
		...
		$this->db->select("PERSON_TYPE_CODE AS OPTION_VALUE, CONCAT(PERSON_TYPE_NAME , ' (', PERSON_TYPE_CODE, ')') AS OPTION_NAME", false);
		$this->db->where("STATUS_ID", 1);
		$this->db->order_by(2);
		$query = $this->db->get($this->tableName);
		$results = ($query->result_array());
