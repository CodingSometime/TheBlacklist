...
<div class="my-1 p-3 rounded shadow bg-body">
  ...
  <div class="my-1 p-3 rounded">
    ...
    <div class="mb-2 row">
      <label for="_GroupCode" class="col-md-3 col-form-label"><?php echo @lang('GROUP_CODE'); ?></label>
      <div class="col-md-4 has-validation">
        <input type="text" class="form-control" id="_GroupCode" name="groupCode" value="<?php echo @$items->groupCode; ?>" required>
      </div>
    </div>
    <div class="mb-2 row">
      <label for="_GroupName" class="col-md-3 col-form-label"><?php echo @lang('GROUP_NAME'); ?></label>
      <div class="col-md-4 has-validation">
        <input type="text" class="form-control" id="_GroupName" name="groupName" value="<?php echo @$items->groupName; ?>" required>
      </div>
    </div>
    <div class="mb-2 row">
...
<script>
  $(document).ready(function() {
    var baseUrl = "<?php echo base_url(); ?>page/group-company";
    var formId = $("#__RequestVerificationId").val();

    // **************************************************************************
    // !validate data ! allegation type code
    // **************************************************************************
    $("#_GroupCode").keyup(function() {
      $("#error-message").text("");
      $("#buttonSubmit").attr("disabled", false);
      let groupCode = encodeURIComponent(this.value);
      let validateUrl = `${baseUrl}/validate/${formId}/${groupCode}`;
