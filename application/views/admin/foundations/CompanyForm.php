  ...
  <div class="my-1 p-3 rounded">
    ...
    <div class="mb-2 row">
      ...
      <div class="col-md-2 has-validation">
        <input disabled type="text" class="form-control" id="_CompanyCode" name="companyCode" onkeyup="this.value = this.value.toUpperCase();" value="<?php echo @$items->companyCode; ?>" required>
      </div>
    </div>
    <div class="mb-2 row">
      <label for="_CompanyName" class="col-md-3 col-form-label"><?php echo @lang('COMPANY_NAME'); ?></label>
      <div class="col-md-4 has-validation">
        <input disabled type="text" class="form-control" id="_CompanyName" name="companyName" value="<?php echo @$items->companyName; ?>" required>
      </div>
    </div>
    <div class="mb-2 row">
      <label for="_DescriptionEn" class="col-md-3 col-form-label"><?php echo @lang('DESCRIPTION_EN'); ?></label>
      <div class="col-md-4 has-validation">
        <input disabled type="text" class="form-control" id="_DescriptionEn" name="descriptionEn" value="<?php echo @$items->descriptionEn; ?>" required>
      </div>
    </div>
    <div class="mb-2 row">
      <label for="_DescriptionTh" class="col-md-3 col-form-label"><?php echo @lang('DESCRIPTION_TH'); ?></label>
      <div class="col-md-4 has-validation">
        <input disabled type="text" class="form-control" id="_DescriptionTh" name="descriptionTh" value="<?php echo @$items->descriptionTh; ?>" required>
      </div>
    </div>

    <div class="mb-2 row">
      <label for="_DescriptionTh" class="col-md-3 col-form-label"><?php echo @lang('BUSINESS_UNIT_CODE'); ?></label>
      <div class="col-md-4 has-validation">
        <input disabled type="text" class="form-control" id="_DescriptionTh" name="businessUnitCode" value="<?php echo @$items->businessUnitName; ?>" required>
      </div>
    </div>    
    <!-- <div class="mb-2 row">
...
<script>
  $(document).ready(function() {
    ...
    // **************************************************************************
    $("#_CompanyCode").keyup(function() {
      $("#error-message").text("");
      $("#buttonSubmit").attr("disabled", false);
      let companyCode = encodeURIComponent(this.value);
      let validateUrl = `${baseUrl}/validate/${formId}/${companyCode}`;
