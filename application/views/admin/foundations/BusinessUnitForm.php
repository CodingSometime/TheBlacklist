  ...
  <div class="my-1 p-3 rounded">
    ...
    <div class="mb-2 row">
      ...
      <div class="col-sm-2 has-validation">
        <input type="text" class="form-control" id="_BusinessUnitCode" name="businessUnitCode" style="text-transform:uppercase" value="<?php echo @$items->businessUnitCode; ?>" required>
      </div>
    </div>
    <div class="mb-2 row">
      <label for="_BusinessUnitName" class="col-sm-3 col-form-label"><?php echo @lang('BUSINESS_UNIT_NAME'); ?></label>
    ...
    <div class="mb-2 row">
      <label for="_GroupCompanyId" class="col-sm-3 col-form-label"><?php echo @lang('GROUP_COMPANY_ID'); ?></label>
      <div class="col-sm-4 has-validation">
...
<script>
  $(document).ready(function() {
    var baseUrl = "<?php echo base_url(); ?>page/business-unit";
    var formId = $("#__RequestVerificationId").val();

    // **************************************************************************
    // !validate data ! allegation type code
    // **************************************************************************
    $("#_BusinessUnitCode").keyup(function() {
      $("#error-message").text("");
      $("#buttonSubmit").attr("disabled", false);
      let businessUnitCode = encodeURIComponent(this.value);
      let validateUrl = `${baseUrl}/validate/${formId}/${businessUnitCode}`;
