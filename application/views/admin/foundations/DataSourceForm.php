...
<div class="my-1 p-3 rounded shadow bg-body">
  ...
  <div class="my-1 p-3 rounded">
    ...
    <div class="mb-2 row">
      <label for="_Code" class="col-md-3 col-form-label"><?php echo @lang('CODE'); ?></label>
      <div class="col-md-2 has-validation">
        <input type="text" class="form-control" id="_Code" name="code" value="<?php echo @$items->code; ?>" required>
      </div>
    </div>
    <div class="mb-2 row">
      <label for="_Name" class="col-md-3 col-form-label"><?php echo @lang('NAME'); ?></label>
      <div class="col-md-4 has-validation">
        <input type="text" class="form-control" id="_Name" name="name" value="<?php echo @$items->name; ?>">
      </div>
    </div>
    <div class="mb-2 row">
...
<script>
  $(document).ready(function() {
    var baseUrl = "<?php echo base_url(); ?>page/data-source";
    var formId = $("#__RequestVerificationId").val();

    // **************************************************************************
    // !validate data ! allegation type code
    // **************************************************************************
    $("#_DataSourceCode").keyup(function() {
      $("#error-message").text("");
      $("#buttonSubmit").attr("disabled", false);
      let dataSourceCode = encodeURIComponent(this.value);
      let validateUrl = `${baseUrl}/validate/${formId}/${dataSourceCode}`;
