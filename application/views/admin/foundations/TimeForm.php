...
<div class="my-1 p-3 rounded shadow bg-body">
  ...
  <div class="my-1 p-3 rounded">
    ...
    <div class="mb-2 row">
      <label for="_TimeCode" class="col-md-3 col-form-label"><?php echo @lang('TIME_CODE'); ?></label>
      <div class="col-md-2 has-validation">
        <input type="text" class="form-control" id="_TimeCode" name="timeCode" onkeyup="this.value = this.value.toUpperCase();" value="<?php echo @$items->timeCode; ?>">
      </div>
    </div>
    <div class="mb-2 row">
      <label for="_TimeName" class="col-md-3 col-form-label"><?php echo @lang('TIME_NAME'); ?></label>
      <div class="col-md-4 has-validation">
        <input type="text" class="form-control" id="_TimeName" name="timeName" value="<?php echo @$items->timeName; ?>">
      </div>
    </div>
    <div class="mb-2 row">
      <label for="_Description" class="col-md-3 col-form-label"><?php echo @lang('DESCRIPTION'); ?></label>
      <div class="col-md-4 has-validation">
        <input type="text" class="form-control" id="_Description" name="description" value="<?php echo @$items->description; ?>">
      </div>
    </div>
    <div class="mb-2 row">
...
<script>
  $(document).ready(function() {
    var baseUrl = "<?php echo base_url(); ?>page/time";
    var formId = $("#__RequestVerificationId").val();

    // **************************************************************************
    // !validate data ! allegation type code
    // **************************************************************************
    $("#_TimeCode").keyup(function() {
      $("#error-message").text("");
      $("#buttonSubmit").attr("disabled", false);
      let timeCode = encodeURIComponent(this.value);
      let validateUrl = `${baseUrl}/validate/${formId}/${timeCode}`;
