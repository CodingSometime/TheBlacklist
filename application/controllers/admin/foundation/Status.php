...
class Status extends BaseController
{
  ...
  private $view_list = "admin/foundations/StatusList";
  private $view_form = "admin/foundations/StatusForm";

  // formatting breadcrumbs
  private $breadcrumbs = array(
    array("" => null),
    array("ROOT" => null),
    array("TITLE" => "/page/status")
  );
  ...
  private function view($action, $id = null)
  {
    ...
    if (isset($id)) {
      ...
      $items["items"] = $object;
      $displayValue = @$object->statusCode . " : " . @$object->statusName;
  ...
  // route: /page/status/validate/(:id)/(:statusCode)
  // method: GET
  public function validate($id, $statusCode)
  {
    // is duplicate ? then return json for javascript validation
    if ($this->BaseModel->isDuplicate($id, $statusCode)) {
