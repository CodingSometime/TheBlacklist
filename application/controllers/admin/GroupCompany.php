...
class GroupCompany extends BaseController
{
  ...
  private $view_list = "admin/foundations/GroupCompanyList";
  private $view_form = "admin/foundations/GroupCompanyForm";

  // formatting breadcrumbs
  private $breadcrumbs = array(
    array("" => null),
    array("ROOT" => null),
    array("TITLE" => "/page/group-company")
  );
  ...
  private function view($action, $id = null)
  {
    ...
    if (isset($id)) {
      ...
      $items["items"] = $object;
      $displayValue = @$object->groupCode . " : " . @$object->groupName;
  ...
  // route: /page/group-company/validate/(:id)/(:groupCompanyCode)
  // method: GET
  public function validate($id, $groupCompanyCode)
  {
    // is duplicate ? then return json for javascript validation
    if ($this->BaseModel->isDuplicate($id, $groupCompanyCode)) {
